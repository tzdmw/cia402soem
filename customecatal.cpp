...
static int run_ethercat(const char *ifname) {
  ...
  if (ec_init(ifname))
  {
    ...
    if ( ec_config_init(FALSE) > 0 )
    {
      ...
      if(ec_slavecount < EXPECTED_SLAVE)
      {
...
void getDataEcat(xEcatData* data)
{
  ...
  for(int slave = 0; slave < ec_slavecount; slave++) {
    xEcatData* slave_src = (xEcatData*)(ec_slave[slave + 1].inputs);
    if(slave_src)
      data[slave] = *(xEcatData*)(ec_slave[slave + 1].inputs);
  }
  data_mutex.unlock();
...
void setCommandEcat(xEcatCommand* command)
{
  command_mutex.lock();
  for(int slave = 0; slave < ec_slavecount; slave++) {
    xEcatCommand* slave_dest = (xEcatCommand*)(ec_slave[slave + 1].outputs);
    if(slave_dest)
      *(xEcatCommand*)(ec_slave[slave + 1].outputs) = command[slave];
  }
  command_mutex.unlock();
}
